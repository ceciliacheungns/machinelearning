...
* A naive (majority-based) predictor would predict a good quality wine in all cases, because the proportion of wines of good quality is approximately `r round(naiveprob,2)*100`% in the training set. If this predictor was used, then it would have an accuracy of `r round(naiveacc,2)*100`% (estimated using test set). The estimated out of sample error for our kNN classifier, using the best k, is `r round(testPerf$accuracy,2)*100`% (in terms of accuracy), even though this is a definite uplift, it is still not a highly accurate predictor.
