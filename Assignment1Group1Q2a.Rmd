...
author: 'Group 1: Low Siow-Meng, Louise Fallon'
output: pdf_document
---
...
The training and validation set data is used to provide the parameters (means and sample standard deviations) to the z-score function, which will then be used to scale the training, validation and test data with the same normalisation formula.
...
for (i in 1:80) {
    ...
    validationPerf <- rbind(validationPerf, 
                            data.frame(k = i, 
                                       acc = (numTP + numTN) / nrow(validation.X), 
...
bestmodel <- validationPerf[order(-validationPerf$acc),][1,]
...
[plot to be included]
```{r}
##to do: make a graph with the data above.
```

Assuming that true positives and true negatives are equally weighted in this case, we use accuracy as the measure for comparing moedls. If true negatives were more important, we would have used sensitivity and if true positives were more important we would have used sensitivity.

We can see that using this measure, the best k is `r bestmodel$k`, with an accuracy of approximately `r round(bestmodel$acc,2)*100`%
...
A naive predictor would predict a good quality wine in all cases, because the proportion of wines of good quality is approximately `r round(naiveprob,2)*100`%. If this predictor was used, then it would have an accuracy of `r round(naiveacc,2)*100`%.
...
The model with the best k is then retrained with the data from both the test and validation set, this will more accurately reflect the out of sample error than if only the test set was used, because we will eventually train the model on the full sample of data to maximise its performance. The performance of this model can be seen in the table above.
...
As a final step, we then train the model with the best k with all the data (training, test and vslidation), to be used for predictions of future data.

*Discussion of whether this is a good classifier based on: sensitivity to seed, comparison to other ks, comparison to naive estimator*
